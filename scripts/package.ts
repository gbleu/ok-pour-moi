...
async function createPackage(): Promise<boolean> {
  ...
  const result = await $`cd dist && zip -r ../${zipFilename} . -x "*.DS_Store" "*.map" "*/.gitkeep" "Thumbs.db"`.quiet();
  ...
  // Count files
  const fileCount = files.filter((line) => !line.includes("Archive:") && line.trim().length > 0).length - 2; // Subtract header lines
