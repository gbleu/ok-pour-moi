...
import { navigateToFolder } from "./outlook-actions.js";
...
async function runWorkflow(config: WorkflowConfig): Promise<WorkflowResult> {
  ...
  try {
    console.log(`[OPM] Navigating to folder "${config.outlookFolder}"...`);
    await navigateToFolder(config.outlookFolder);
...
document.addEventListener("keydown", (event) => {
  if (event.ctrlKey && event.shiftKey && (event.key === "O" || event.key === "o")) {
    console.log("[OPM] Debug trigger: Ctrl+Shift+O pressed - use popup to start workflow");
  }
});
