...
Since the extension only stores data locally using `chrome.storage.local` and doesn't transmit anything to external servers.
...
1. **`storage`**
   ```
   Used to save the user's signature image and configuration settings (email, 
   reply message, signature position) locally on the device. No data is synced 
...
4. **`downloads`**
   ```
   Enables downloading PDF attachments from Outlook for signing and saving 
   signed PDFs back to the user's device. Downloads are only triggered when 
   the user explicitly clicks "Sign PDFs & Create Drafts".
   ```

5. **Host Permissions** (outlook.office365.com, etc.)
   ```
